...
const MyCard = () => {
  ...
  //get product cart
  const { data: product, loading } = useSelector((state) => state.cartState);
  const productCart = product.cartItems ?? [];
  const productSelected = checkProductSelected(product.cartItems);
  const moneyShip = sumMoneyShip(product.cartItems);
  const sum = sumMoneyProduct(product.cartItems);
  const arrayId = createArrayId(product.cartItems) ?? [];
  ...
  // delete cart items
  const handleDeleteCartItem = async (id, type) => {
    let newArrayId = [];
    if (Array.isArray(id)) {
      newArrayId.push(id);
    } else {
    ...
    if (type !== "many") {
      try {
        await dispatch(deleteCartUser({ newArrayId })).unwrap();
      ...
      toast.error("Vui lòng chọn sản phẩm xóa!");
    }
  };
    ...
    if (quantity - 1 > 0) {
      try {
        await dispatch(
          addCartUser({
            cartItems: [
              { product: product, quantity: quantity - 1, selected: selected },
            ],
          })
        ).unwrap();
  ...
  const handleChangeChecked = async (product, quantity, selected) => {
    try {
      await dispatch(
        addCartUser({
          cartItems: [{ product, quantity, selected }],
        })
      ).unwrap();
      setRefreshKey((oldKey) => oldKey + 1);
    } catch (err) {
          ...
          <div className="row sm-gutter app__content">
            <div className="my--card">
              <>
                    ...
                    <div className="cart-gateway--info">
                      <div className="cart-gateway--info__model">
                        <div className="cart-input">
                          ...
                          <h3>TẤT CẢ ({productCart.length} SẢN PHẨM)</h3>
                        </div>
                      </div>
                      {arrayId.length === productCart.length ? (
                        <div
                          className="cart-delete"
                          onClick={() => {
                            handleDeleteCartItem(arrayId, "many");
                          }}
                        >
                        ...
                        >
                          <i className="fa-solid fa-trash"></i>
                          <span>XÓA ({arrayId.length})</span>
                        </div>
                      )}
                    </div>
                    {productCart.length > 0 || loading === true
                              ...
                              <input
                                ...
                                onChange={() => {
                                  handleChangeChecked(
                                    item.product._id,
                                    item.quantity,
                                    !product.cartItems[i].selected
                                  );
                                }}
                              />
                            </div>
                            <CartItem productCart={item.product} />
                    ...
                    <div className="cart-wrap--method__info">
                      ...
                      <div className="cart-wrap--method__info--cal">
                        ...
                        <div className="cal-submit">
                          ...
                          >
                            XÁC NHẬN GIỎ HÀNG <span>({productSelected})</span>
                          </button>
                        </div>
                      </div>
                    </div>
                ...
                {loading === true && productCart.length === 0 && <Loader />}
              </>
            </div>
          </div>
        </div>
